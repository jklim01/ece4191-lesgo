...
void us_clk_Start(void) 
{
    ...
    us_clk_CLKEN |= us_clk_CLKEN_MASK;
	us_clk_CLKSTBY |= us_clk_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: us_clk_Stop
********************************************************************************
*
* Summary:
...
void us_clk_Stop(void) 
{
    ...
    us_clk_CLKEN &= (uint8)(~us_clk_CLKEN_MASK);
	us_clk_CLKSTBY &= (uint8)(~us_clk_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: us_clk_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((us_clk_CLKEN & us_clk_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = us_clk_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = us_clk_CLKEN_MASK;
        ...
        oldDivider = CY_GET_REG16(us_clk_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(us_clk_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: us_clk_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: us_clk_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = us_clk_GetSourceRegister();
    uint16 oldDivider = us_clk_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = us_clk_CLKEN & us_clk_CLKEN_MASK;
        ...
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((us_clk_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
/*******************************************************************************
* Function Name: us_clk_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(us_clk_DIV_PTR);
}


/*******************************************************************************
* Function Name: us_clk_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    us_clk_MOD_SRC |= modeBitMask & (uint8)us_clk_MODE_MASK;
}


/*******************************************************************************
* Function Name: us_clk_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    us_clk_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(us_clk_MODE_MASK));
}


/*******************************************************************************
* Function Name: us_clk_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return us_clk_MOD_SRC & (uint8)(us_clk_MODE_MASK);
}


/*******************************************************************************
* Function Name: us_clk_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = us_clk_GetDividerRegister();
    uint8 oldSrc = us_clk_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        us_clk_MOD_SRC |= CYCLK_SSS;
        us_clk_MOD_SRC =
            (us_clk_MOD_SRC & (uint8)(~us_clk_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: us_clk_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return us_clk_MOD_SRC & us_clk_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: us_clk_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    us_clk_PHASE = clkPhase & us_clk_PHASE_MASK;
}


/*******************************************************************************
* Function Name: us_clk_GetPhase
********************************************************************************
*
* Summary:
...
{
    return us_clk_PHASE & us_clk_PHASE_MASK;
}
