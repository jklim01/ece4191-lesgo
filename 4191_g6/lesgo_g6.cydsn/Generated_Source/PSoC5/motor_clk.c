...
void motor_clk_Start(void) 
{
    ...
    motor_clk_CLKEN |= motor_clk_CLKEN_MASK;
	motor_clk_CLKSTBY |= motor_clk_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: motor_clk_Stop
********************************************************************************
*
* Summary:
...
void motor_clk_Stop(void) 
{
    ...
    motor_clk_CLKEN &= (uint8)(~motor_clk_CLKEN_MASK);
	motor_clk_CLKSTBY &= (uint8)(~motor_clk_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: motor_clk_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((motor_clk_CLKEN & motor_clk_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = motor_clk_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = motor_clk_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* motor_clk__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(motor_clk_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(motor_clk_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: motor_clk_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: motor_clk_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = motor_clk_GetSourceRegister();
    uint16 oldDivider = motor_clk_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = motor_clk_CLKEN & motor_clk_CLKEN_MASK;
        ...
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((motor_clk_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
/*******************************************************************************
* Function Name: motor_clk_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(motor_clk_DIV_PTR);
}


/*******************************************************************************
* Function Name: motor_clk_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    motor_clk_MOD_SRC |= modeBitMask & (uint8)motor_clk_MODE_MASK;
}


/*******************************************************************************
* Function Name: motor_clk_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    motor_clk_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(motor_clk_MODE_MASK));
}


/*******************************************************************************
* Function Name: motor_clk_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return motor_clk_MOD_SRC & (uint8)(motor_clk_MODE_MASK);
}


/*******************************************************************************
* Function Name: motor_clk_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = motor_clk_GetDividerRegister();
    uint8 oldSrc = motor_clk_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        motor_clk_MOD_SRC |= CYCLK_SSS;
        motor_clk_MOD_SRC =
            (motor_clk_MOD_SRC & (uint8)(~motor_clk_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: motor_clk_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return motor_clk_MOD_SRC & motor_clk_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: motor_clk_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    motor_clk_PHASE = clkPhase & motor_clk_PHASE_MASK;
}


/*******************************************************************************
* Function Name: motor_clk_GetPhase
********************************************************************************
*
* Summary:
...
{
    return motor_clk_PHASE & motor_clk_PHASE_MASK;
}
