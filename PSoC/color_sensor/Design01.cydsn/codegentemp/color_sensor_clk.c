...
/*******************************************************************************
* Function Name: color_sensor_clk_Start
********************************************************************************
*
* Summary:
...
void color_sensor_clk_Start(void) 
{
    ...
    color_sensor_clk_CLKEN |= color_sensor_clk_CLKEN_MASK;
	color_sensor_clk_CLKSTBY |= color_sensor_clk_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: color_sensor_clk_Stop
********************************************************************************
*
* Summary:
...
void color_sensor_clk_Stop(void) 
{
    ...
    color_sensor_clk_CLKEN &= (uint8)(~color_sensor_clk_CLKEN_MASK);
	color_sensor_clk_CLKSTBY &= (uint8)(~color_sensor_clk_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: color_sensor_clk_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((color_sensor_clk_CLKEN & color_sensor_clk_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = color_sensor_clk_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = color_sensor_clk_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* color_sensor_clk__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(color_sensor_clk_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(color_sensor_clk_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: color_sensor_clk_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: color_sensor_clk_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = color_sensor_clk_GetSourceRegister();
    uint16 oldDivider = color_sensor_clk_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = color_sensor_clk_CLKEN & color_sensor_clk_CLKEN_MASK;
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                CY_SET_REG16(color_sensor_clk_DIV_PTR, clkDivider);
                color_sensor_clk_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
                /* Moving to SSS, set SSS which then ignores the divider and we can set     */
                /* it without bothering with the shadow load.                               */
                color_sensor_clk_MOD_SRC |= CYCLK_SSS;
                CY_SET_REG16(color_sensor_clk_DIV_PTR, clkDivider);
            }
        }
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((color_sensor_clk_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
            ...
            else
            {
                ...
                CY_SET_REG16(color_sensor_clk_DIV_PTR, clkDivider);
				color_sensor_clk_CLKEN |= enabled;
            }
        }
    }
}
...
/*******************************************************************************
* Function Name: color_sensor_clk_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(color_sensor_clk_DIV_PTR);
}


/*******************************************************************************
* Function Name: color_sensor_clk_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    color_sensor_clk_MOD_SRC |= modeBitMask & (uint8)color_sensor_clk_MODE_MASK;
}


/*******************************************************************************
* Function Name: color_sensor_clk_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    color_sensor_clk_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(color_sensor_clk_MODE_MASK));
}


/*******************************************************************************
* Function Name: color_sensor_clk_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return color_sensor_clk_MOD_SRC & (uint8)(color_sensor_clk_MODE_MASK);
}


/*******************************************************************************
* Function Name: color_sensor_clk_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = color_sensor_clk_GetDividerRegister();
    uint8 oldSrc = color_sensor_clk_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        color_sensor_clk_MOD_SRC |= CYCLK_SSS;
        color_sensor_clk_MOD_SRC =
            (color_sensor_clk_MOD_SRC & (uint8)(~color_sensor_clk_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: color_sensor_clk_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return color_sensor_clk_MOD_SRC & color_sensor_clk_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: color_sensor_clk_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    color_sensor_clk_PHASE = clkPhase & color_sensor_clk_PHASE_MASK;
}


/*******************************************************************************
* Function Name: color_sensor_clk_GetPhase
********************************************************************************
*
* Summary:
...
{
    return color_sensor_clk_PHASE & color_sensor_clk_PHASE_MASK;
}
