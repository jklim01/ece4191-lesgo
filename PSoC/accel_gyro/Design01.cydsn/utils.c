...
extern const uint8 MPU6050;
extern const uint8 PWR_MGMT_1;
extern const uint8 GYRO_CONFIG;
extern const uint8 CONFIG;
extern const uint8 GYRO_XOUT_H;
...
void setMemoryBank(uint8 bank, bool prefetchEnabled, bool userBank) {
    ...
    I2C_1_MasterWriteByte(0x6D);
    I2C_1_MasterWriteByte(bank);
...
void setMemoryStartAddress(uint8_t address) {
    ...
    I2C_1_MasterSendStart(MPU6050, 0);
    I2C_1_MasterWriteByte(0x6E);
    I2C_1_MasterWriteByte(address);
    I2C_1_MasterSendStop();
}

bool writeProgMemoryBlock(const uint8_t *data, uint16_t dataSize, uint8_t bank, uint8_t address, bool verify, bool useProgMem) {
    // bank = 0;
    // address = 0;
    // verify = true;
    // useProgMem = true;
    ...
    uint8* verifyBuffer=0;
    uint8* progBuffer=0;
    if (verify) verifyBuffer = (uint8_t *)malloc(MPU6050_DMP_MEMORY_CHUNK_SIZE);
    if (useProgMem) progBuffer = (uint8_t *)malloc(MPU6050_DMP_MEMORY_CHUNK_SIZE);
    for (uint16 i = 0; i < dataSize;) {
        // determine correct chunk size according to bank position and data size
        uint8 chunkSize = MPU6050_DMP_MEMORY_CHUNK_SIZE;
        ...
        if (useProgMem) {
            ...
            for (uint8 j = 0; j < chunkSize; j++) progBuffer[j] = *(uint8*)(data + i + j);
        } else {
            // write the chunk of data as specified
        ...
        I2C_1_MasterSendStart(MPU6050, 0);
        I2C_1_MasterWriteByte(0x6F);
        I2C_1_MasterWriteByte(chunkSize);
        I2C_1_MasterSendStop();
        ...
        if (verify && verifyBuffer) {
            ...
            for (uint8 i = 0; i < chunkSize-1; i++)
                verifyBuffer[i] = I2C_1_MasterReadByte(I2C_1_ACK_DATA);
            verifyBuffer[chunkSize-1] = I2C_1_MasterReadByte(I2C_1_NAK_DATA);
